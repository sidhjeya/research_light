...
def processing():
    ...
    depth_frame=rs.get_depth_frame()
    while True:
        try:
            ...
            point_1 = rs2.rs2_deproject_pixel_to_point(intr, xA, yA)
            point_2 = rs2.rs2_deproject_pixel_to_point(intr, point_tr[0], point_tr[1])
            point_3 = rs2.rs2_deproject_pixel_to_point(intr, point_bl[0], point_bl[1])
            point_4 = rs2.rs2_deproject_pixel_to_point(intr, xB, yB)
            length = math.sqrt((point_1[0] - point_2[0])**2 + (point_1[1] - point_2[1])**2 + (point_1[2] - point_2[2])**2)
            width = math.sqrt((point_1[0] - point_3[0])**2 + (point_1[1] - point_3[1])**2 + (point_1[2] - point_3[2])**2)
            # print("length ----------------------->",point_1)
            print("width------------------------>",width)
            if classs == 0 :
                  object = 'CARPET'
            if classs == 1 :
